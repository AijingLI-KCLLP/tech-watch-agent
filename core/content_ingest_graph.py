...
from adapters.store import link_input_asset_to_article, save_article, save_article_tags, save_chunks
...
def tag_node(state: ContentIngestState) -> dict:
    return {"tags": tag_article(state["article"])}
...
def store_node(state: ContentIngestState) -> dict:
    ...
    save_article_tags(persisted_article_id, state["tags"])
